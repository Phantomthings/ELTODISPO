...
type bloc struct {
	...
	est_disponible int
	cause          *string
	icpc           *string // Nouveau champ ICPC
	rawPointCount  int
	batchID        string
	hashSig        string
}
...
// buildICPC construit la valeur ICPC pour tous les blocs
func buildICPC(s sample) string {
	pc := "NA"
	ic := "NA"
	if s.pc1 != nil {
		pc = strconv.Itoa(*s.pc1)
	...
	return fmt.Sprintf("IC1=%s,PC1=%s", ic, pc)
}
...
func buildBlocks(site string, cfg pdcConfig, series map[time.Time]sample, batchID string) []bloc {
	...
	for i := 1; i < len(rows); i++ {
		...
		if r.state != curState || causeKey(r.cause) != causeKey(curCause) {
			...
			icpcVal := buildICPC(rows[i-1].val) // Utilise la dernière valeur du bloc
			blocks = append(blocks, bloc{
				site:           site,
				pdcID:          cfg.Label,
				dateDebut:      start,
				dateFin:        end,
				est_disponible: curState,
				cause:          curCause,
				icpc:           &icpcVal, // Toujours rempli
	...
	if curState != 0 || int(end.Sub(start).Minutes()) >= minIndispoMinutes {
		h := hashSig(site, cfg.Label, start, end, curState, curCause)
		icpcVal := buildICPC(rows[len(rows)-1].val)
		blocks = append(blocks, bloc{
			site:           site,
			pdcID:          cfg.Label,
			dateDebut:      start,
...
func upsertStmt(tbl string) string {
	return fmt.Sprintf(`
INSERT INTO %s
(site, pdc_id, type_label, date_debut, date_fin, est_disponible, cause, ICPC, raw_point_count, processed_at, batch_id, hash_signature)
...
func saveBlocks(db *sql.DB, site string, cfg pdcConfig, blocks []bloc) (int, error) {
	...
	for _, b := range blocks {
		...
		icpc := sql.NullString{}
		if b.icpc != nil {
			icpc = sql.NullString{String: *b.icpc, Valid: true}
		}
		startStore := b.dateDebut.In(paris)
		endStore := b.dateFin.In(paris)
		args := []any{b.site, b.pdcID, startStore, endStore, b.est_disponible, cause, icpc, b.rawPointCount, b.batchID, b.hashSig}
...
// Worker traite les items de travail depuis un canal
func worker(id int, jobs <-chan workItem, results chan<- int, wg *sync.WaitGroup, ic influx.Client, db *sql.DB) {
	defer wg.Done()
	for job := range jobs {
		n, err := processDayParis(job.site, job.cfg, job.day, ic, db)
		if err != nil {
			log.Printf("[Worker %d] Erreur %s %s %s: %v", id, job.site, job.cfg.Label, job.day.Format("2006-01-02"), err)
			continue
		}
		results <- n
		time.Sleep(50 * time.Millisecond) // Throttle léger
	}
}
...
func autoUpdateAll(projects []string) error {
	...
	// Lancer les workers
	log.Printf("Démarrage de %d workers...", numWorkers)
	for w := 1; w <= numWorkers; w++ {
		wg.Add(1)
		ic := influxClient()
		db := mysqlDBConn()
		go func(workerID int, client influx.Client, database *sql.DB) {
			defer client.Close()
			defer database.Close()
			worker(workerID, jobs, results, &wg, client, database)
		}(w, ic, db)
	}
	...
	// Connexion pour la préparation des jobs
	ic := influxClient()
	defer ic.Close()
	db := mysqlDBConn()
	defer db.Close()
	...
	for _, site := range projects {
		for _, cfg := range pdcConfigs {
			...
			for d := startDay; !d.After(yesterdayP); d = d.Add(24 * time.Hour) {
				jobs <- workItem{site: site, cfg: cfg, day: d}
			}
		}
		log.Printf("Jobs créés pour %s", site)
	}

	// Fermer le canal des jobs et attendre les workers
	close(jobs)
	wg.Wait()
	close(results)
