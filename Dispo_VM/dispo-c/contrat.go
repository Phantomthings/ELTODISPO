...
type blocRecord struct {
	...
	dateDebut time.Time
	dateFin   time.Time
	etat      int // 1=up, 0=down, -1=pas de données
}
...
// Charge les blocs PDC (avec est_disponible et pdc_id au lieu de equipement_id)
...
func calculateMonthly(db *sql.DB, site string, year int, month time.Month) error {
	...
	availabilityPct := 0.0
	if t2Count > 0 {
		availabilityPct = (tSum / float64(t2Count)) * 100
	}

	// Sauvegarde (t3=0 car plus utilisé, mais colonne existe encore)
	upsertQuery := `
	...
	_, err = db.Exec(upsertQuery, site, periodStart.Format("2006-01-02"), t2Count, tSum, availabilityPct)
	if err != nil {
		return fmt.Errorf("upsert result: %w", err)
	}

	log.Printf("%s %d-%02d: T2=%d, T_sum=%.2f, Dispo=%.2f%%",
		site, year, month, t2Count, tSum, availabilityPct)
