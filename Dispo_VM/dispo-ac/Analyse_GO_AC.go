// dispo_ac with ICPC field and parallel workers
package main
...
type bloc struct {
	...
	estDispo      int
	cause         *string
	icpc          *string // NEW: Store IC1/PC1 values
	rawPointCount int
	batchID       string
	hashSig       string
}
...
// Classification
func classifyAC(pc1, ic1 *int) (int, *string, *string) {
	// NEW: Always create ICPC string if we have values
	var icpc *string
	if pc1 != nil && ic1 != nil {
		s := fmt.Sprintf("IC1=%d,PC1=%d", *ic1, *pc1)
		icpc = &s
	}
...
func buildBlocksAC(site string, series map[time.Time]pair, batchID string) []bloc {
	...
	for t, v := range series {
		st, cause, icpc := classifyAC(v.pc1, v.ic1)
		rows = append(rows, row{t: t, state: st, cause: cause, icpc: icpc, pc1: v.pc1, ic1: v.ic1})
	}
...
func upsertStmtAC(t string) string {
	return fmt.Sprintf(`
INSERT INTO %s
(site, equipement_id, type_equipement, date_debut, date_fin, est_disponible, cause, ICPC,
...
func saveBlocksPerSiteAC(db *sql.DB, site string, blocks []bloc) (int, error) {
	...
	for _, b := range blocks {
		...
		args := []any{b.site, startStore, endStore, b.estDispo, cause, icpc, b.rawPointCount, b.batchID, b.hashSig}
...
func worker(id int, jobs <-chan workItem, results chan<- int, wg *sync.WaitGroup, ic influx.Client, db *sql.DB) {
	defer wg.Done()
	for job := range jobs {
		n, err := processDayParisAC(job.site, job.dayParis, ic, db)
		if err != nil {
			log.Printf("[Worker %d] Erreur %s %s: %v", id, job.site, job.dayParis.Format("2006-01-02"), err)
			results <- 0
			continue
		}
		results <- n
		time.Sleep(50 * time.Millisecond) // Reduced sleep for parallel processing
	}
}
...
func autoUpdateAllAC(projects []string) error {
	...
	// Start workers
	log.Printf("Démarrage de %d workers parallèles", numWorkers)
	for w := 1; w <= numWorkers; w++ {
		wg.Add(1)
		go worker(w, jobs, results, &wg, ic, db)
	}

	// Collect jobs
	var allJobs []workItem
	for _, site := range projects {
		...
		for d := startDayP; !d.After(yesterdayP); d = d.Add(24 * time.Hour) {
			allJobs = append(allJobs, workItem{site: site, dayParis: d})
		}
	}

	log.Printf("Total de %d jours à traiter", len(allJobs))
	...
	log.Printf("Terminé: %d blocs AC traités par %d workers", totalAC, numWorkers)
	return nil
}
...
func main() {
	...
	workers := flag.Int("workers", numWorkers, "Nombre de workers parallèles")
	flag.Parse()
	...
	// Mode autoupdate with parallel workers
